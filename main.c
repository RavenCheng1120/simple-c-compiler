...
void stmt()
{
  ...
  if (tk == If) {
    ...
    *b = (int)(e + 1);
		...
		if (tempd1[Class] == Num) { *++e = IMM; *++e = tempd1[Val]; ty = INT; }
	    else {
	      if (tempd1[Class] == Loc) { *++e = LEA; *++e = loc - tempd1[Val]; }
	      else if (tempd1[Class] == Glo) { *++e = IMM; *++e = tempd1[Val]; }
	      else { printf("%d: undefined variable\n", line); exit(-1); }
	      *++e = ((ty = tempd1[Type]) == CHAR) ? LC : LI;
	    }
	    if (*e == LC) { *e = PSH; *++e = LC; }
		else if (*e == LI) { *e = PSH; *++e = LI; }
		else { printf("%d: bad lvalue in pre-increment\n", line); exit(-1); }
...
int main(int argc, char **argv) {
    ...
    memset(sym,  0, poolsize);
  	...
  	//store the Keywords such as if, while or for in the symbol table
    p = "char else enum if int return sizeof for while "
      "open read close printf malloc free memset memcmp exit void main";
    i = Char;
	while (i <= While){ // add 8 keywords to symbol table
		next(); 
		id[Tk] = i++;
	}
	i = OPEN; 
	while (i <= EXIT){ // add 9 library to symbol table
		next(); 
		id[Class] = Sys; 
		id[Type] = INT;
		id[Val] = i++; 
  ...
  *--sp = (int)t;
  ...
  while (1) {
    ...
    else { printf("unknown instruction"); return -1; }
  }
}
